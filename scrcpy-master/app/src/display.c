...
static bool sc_display_init_novideo_icon(struct sc_display *display,
                                         SDL_Surface *icon_novideo) {
  assert(icon_novideo);

  if (SDL_RenderSetLogicalSize(display->renderer, icon_novideo->w,
                               icon_novideo->h)) {
  ...
  display->texture =
      SDL_CreateTextureFromSurface(display->renderer, icon_novideo);
  if (!display->texture) {
...
bool sc_display_init(struct sc_display *display, SDL_Window *window,
...
static SDL_Texture *sc_display_create_texture(struct sc_display *display,
                                              struct sc_size size) {
...
static inline void sc_display_set_pending_size(struct sc_display *display,
                                               struct sc_size size) {
...
static bool sc_display_set_pending_frame(struct sc_display *display,
                                         const AVFrame *frame) {
  if (!display->pending.frame) {
...
// Forward declaration
static bool sc_display_update_texture_internal(struct sc_display *display,
                                               const AVFrame *frame);
...
static bool sc_display_apply_pending(struct sc_display *display) {
  ...
  if (display->pending.flags & SC_DISPLAY_PENDING_FLAG_FRAME) {
    ...
    bool ok =
        sc_display_update_texture_internal(display, display->pending.frame);
    if (!ok) {
      return false;
    }
...
static bool sc_display_update_texture_internal(struct sc_display *display,
  ...
  int ret = SDL_UpdateYUVTexture(
      display->texture, NULL, frame->data[0], frame->linesize[0],
...
enum sc_display_result sc_display_update_texture(struct sc_display *display,
                                                 const AVFrame *frame) {
...
enum sc_display_result sc_display_render(struct sc_display *display,
  ...
  if (orientation == SC_ORIENTATION_0) {
    ...
    SDL_RendererFlip flip =
        sc_orientation_is_mirror(orientation) ? SDL_FLIP_HORIZONTAL : 0;
  ...
  // SDL_RenderPresent is called from screen.c after keymapper overlay
  return SC_DISPLAY_RESULT_OK;
}
