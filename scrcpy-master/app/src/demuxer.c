...
static bool sc_demuxer_recv_codec_id(struct sc_demuxer *demuxer,
                                     uint32_t *codec_id) {
  uint8_t data[4];
...
static bool sc_demuxer_recv_video_size(struct sc_demuxer *demuxer,
...
static bool sc_demuxer_recv_packet(struct sc_demuxer *demuxer,
                                   AVPacket *packet) {
...
static int run_demuxer(void *data) {
  ...
  bool must_merge_config_packet =
      raw_codec_id == SC_CODEC_ID_H264 || raw_codec_id == SC_CODEC_ID_H265;
...
void sc_demuxer_init(struct sc_demuxer *demuxer, const char *name,
...
bool sc_demuxer_start(struct sc_demuxer *demuxer) {
...
void sc_demuxer_join(struct sc_demuxer *demuxer) {
