...
void sc_input_manager_init(struct sc_input_manager *im,
  ...
  // KEYMAPPER: Initialize overlay and load config
  km_init();
}

static void send_keycode(struct sc_input_manager *im,
                         enum android_keycode keycode, enum sc_action action,
                         const char *name) {
...
static inline void action_home(struct sc_input_manager *im,
                               enum sc_action action) {
  send_keycode(im, AKEYCODE_HOME, action, "HOME");
}

static inline void action_back(struct sc_input_manager *im,
                               enum sc_action action) {
  send_keycode(im, AKEYCODE_BACK, action, "BACK");
}

static inline void action_app_switch(struct sc_input_manager *im,
                                     enum sc_action action) {
  send_keycode(im, AKEYCODE_APP_SWITCH, action, "APP_SWITCH");
}

static inline void action_power(struct sc_input_manager *im,
                                enum sc_action action) {
  send_keycode(im, AKEYCODE_POWER, action, "POWER");
}

static inline void action_volume_up(struct sc_input_manager *im,
                                    enum sc_action action) {
  send_keycode(im, AKEYCODE_VOLUME_UP, action, "VOLUME_UP");
}

static inline void action_volume_down(struct sc_input_manager *im,
                                      enum sc_action action) {
  send_keycode(im, AKEYCODE_VOLUME_DOWN, action, "VOLUME_DOWN");
}

static inline void action_menu(struct sc_input_manager *im,
                               enum sc_action action) {
...
static void press_back_or_turn_screen_on(struct sc_input_manager *im,
                                         enum sc_action action) {
  assert(im->controller && im->kp);
...
static void expand_notification_panel(struct sc_input_manager *im) {
...
static void expand_settings_panel(struct sc_input_manager *im) {
  assert(im->controller);
...
static void collapse_panels(struct sc_input_manager *im) {
  assert(im->controller);
...
static bool get_device_clipboard(struct sc_input_manager *im,
...
static bool set_device_clipboard(struct sc_input_manager *im, bool paste,
                                 uint64_t sequence) {
  assert(im->controller && im->kp);
...
static void set_display_power(struct sc_input_manager *im, bool on) {
  assert(im->controller);
...
static void switch_fps_counter_state(struct sc_input_manager *im) {
...
static void clipboard_paste(struct sc_input_manager *im) {
  assert(im->controller && im->kp);
...
static void rotate_device(struct sc_input_manager *im) {
  assert(im->controller);
...
static void open_hard_keyboard_settings(struct sc_input_manager *im) {
  assert(im->controller);
...
static void reset_video(struct sc_input_manager *im) {
  assert(im->controller);
...
static void apply_orientation_transform(struct sc_input_manager *im,
...
static bool simulate_virtual_finger(struct sc_input_manager *im,
...
static struct sc_point inverse_point(struct sc_point point, struct sc_size size,
...
static void sc_input_manager_process_key(struct sc_input_manager *im,
  ...
  if (is_shortcut) {
    ...
    case SDLK_r:
      ...
      return;
    case SDLK_k:
      if (control && !shift && !repeat && down && !paused && im->kp &&
          im->kp->hid) {
  ...
  if (im->clipboard_autosync && is_ctrl_v) {
    ...
    uint64_t sequence =
        im->kp->async_paste ? im->next_sequence : SC_SEQUENCE_INVALID;
  ...
  // ==========================================
  // KEYMAPPER: F12 toggle edit, PgUp/PgDn opacity
  // ==========================================
  if (down && !repeat && sdl_keycode == SDLK_F12) {
    km_toggle_edit();
    return;
  }
  if (down && sdl_keycode == SDLK_PAGEUP) {
    km_adjust_opacity(0.1f);
    return;
  }
  if (down && sdl_keycode == SDLK_PAGEDOWN) {
    km_adjust_opacity(-0.1f);
    return;
  }
  ...
  if (down && !repeat && sdl_keycode == SDLK_F5) {
    km_reload_config();
    return;
  }
  if (down && !repeat && sdl_keycode == SDLK_F11) {
    km_toggle_overlay();
    return;
  }
  // Delete selected binding in edit mode
  struct km_state *kms = km_get_state();
  if (kms->edit_mode && down && sdl_keycode == SDLK_DELETE &&
      kms->selected >= 0) {
    for (int j = kms->selected; j < kms->count - 1; j++) {
      kms->bindings[j] = kms->bindings[j + 1];
    }
    kms->count--;
    kms->selected = -1;
    return;
  }
  // KEYMAPPER DINAMICO: intercept mapped keys
  struct km_binding *kb = km_find_key(sdl_keycode);
  if (kb && im->controller && !kms->edit_mode) {
    struct sc_point touch_pt;
    touch_pt.x = (int32_t)(im->screen->frame_size.width * kb->x_pct);
    touch_pt.y = (int32_t)(im->screen->frame_size.height * kb->y_pct);

    struct sc_control_msg touch_msg;
    touch_msg.type = SC_CONTROL_MSG_TYPE_INJECT_TOUCH_EVENT;
    touch_msg.inject_touch_event.action =
        down ? AMOTION_EVENT_ACTION_DOWN : AMOTION_EVENT_ACTION_UP;
    touch_msg.inject_touch_event.pointer_id = kb->pointer_id;
    touch_msg.inject_touch_event.position.screen_size = im->screen->frame_size;
    touch_msg.inject_touch_event.position.point = touch_pt;
    touch_msg.inject_touch_event.pressure = down ? 1.0f : 0.0f;
    touch_msg.inject_touch_event.action_button = 0;
    touch_msg.inject_touch_event.buttons = 0;

    sc_controller_push_msg(im->controller, &touch_msg);
    return;
  }
  // ==========================================
...
sc_input_manager_process_mouse_motion(struct sc_input_manager *im,
  ...
  // KEYMAPPER: drag handling in edit mode
  struct km_state *kms_m = km_get_state();
  if (kms_m->edit_mode && kms_m->dragging >= 0) {
    SDL_Event ev;
    ev.type = SDL_MOUSEMOTION;
    ev.motion = *event;
    km_handle_edit_mouse(&ev, &im->screen->rect, im->screen->frame_size);
    return;
  }
  ...
  struct sc_mouse_motion_event evt = {
      ...
      .pointer_id =
          im->vfinger_down ? SC_POINTER_ID_GENERIC_FINGER : SC_POINTER_ID_MOUSE,
      .xrel = event->xrel,
      .yrel = event->yrel,
  ...
  if (im->vfinger_down) {
    ...
    struct sc_point vfinger =
        inverse_point(mouse, im->screen->frame_size, im->vfinger_invert_x,
                      im->vfinger_invert_y);
    simulate_virtual_finger(im, AMOTION_EVENT_ACTION_MOVE, vfinger);
  }
}

static void sc_input_manager_process_touch(struct sc_input_manager *im,
...
sc_input_manager_process_mouse_button(struct sc_input_manager *im,
  ...
  // KEYMAPPER: Edit mode mouse handling
  struct km_state *kms = km_get_state();
  if (kms->edit_mode) {
    SDL_Event ev;
    ev.type = (event->type == SDL_MOUSEBUTTONDOWN) ? SDL_MOUSEBUTTONDOWN
                                                   : SDL_MOUSEBUTTONUP;
    ev.button = *event;
    if (km_handle_edit_mouse(&ev, &im->screen->rect, im->screen->frame_size)) {
      return;
    }
  }
  ...
  if (mb && im->controller && !kms->edit_mode) {
    ...
    touch_msg.inject_touch_event.action =
        down ? AMOTION_EVENT_ACTION_DOWN : AMOTION_EVENT_ACTION_UP;
    touch_msg.inject_touch_event.pointer_id = mb->pointer_id;
    touch_msg.inject_touch_event.position.screen_size = im->screen->frame_size;
    touch_msg.inject_touch_event.position.point = touch_pt;
    touch_msg.inject_touch_event.pressure = down ? 1.0f : 0.0f;
    touch_msg.inject_touch_event.action_button = 0;
  ...
  if (control && !paused) {
    ...
    struct sc_mouse_binding_set *bindings =
        !shift_pressed ? &im->mouse_bindings.pri : &im->mouse_bindings.sec;
  ...
  struct sc_mouse_click_event evt = {
      ...
      .button = button,
      .pointer_id =
          use_finger ? SC_POINTER_ID_GENERIC_FINGER : SC_POINTER_ID_MOUSE,
      .buttons_state = im->mouse_buttons_state,
  };
...
static void sc_input_manager_process_gamepad_device(
    struct sc_input_manager *im, const SDL_ControllerDeviceEvent *event) {
...
static void sc_input_manager_process_gamepad_button(
    struct sc_input_manager *im, const SDL_ControllerButtonEvent *event) {
...
static void sc_input_manager_process_file(struct sc_input_manager *im,
