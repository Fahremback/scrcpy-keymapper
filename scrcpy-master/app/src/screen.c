...
static struct sc_size get_window_size(const struct sc_screen *screen) {
  int width;
  int height;
...
static struct sc_point get_window_position(const struct sc_screen *screen) {
  int x;
  int y;
...
static void set_window_size(struct sc_screen *screen, struct sc_size new_size) {
...
static bool get_preferred_display_bounds(struct sc_size *bounds) {
  SDL_Rect rect;
...
static bool is_optimal_size(struct sc_size current_size,
                            struct sc_size content_size) {
  // The size is optimal if we can recompute one dimension of the current
  // size from the other
  return current_size.height ==
             current_size.width * content_size.height / content_size.width ||
         current_size.width ==
             current_size.height * content_size.width / content_size.height;
}
...
static struct sc_size get_optimal_size(struct sc_size current_size,
                                       struct sc_size content_size,
  ...
  if (!within_display_bounds || !get_preferred_display_bounds(&display_size)) {
  ...
  if (keep_width) {
    ...
    window_size.height =
        content_size.height * window_size.width / content_size.width;
  } else {
    // remove black borders on left and right (or none at all if it already
    // fits)
    window_size.width =
        content_size.width * window_size.height / content_size.height;
  }
...
get_initial_optimal_size(struct sc_size content_size, uint16_t req_width,
  ...
  if (!req_width && !req_height) {
    ...
    if (req_width) {
      ...
      window_size.width =
          (uint32_t)req_height * content_size.width / content_size.height;
    }
    if (req_height) {
      ...
      window_size.height =
          (uint32_t)req_width * content_size.height / content_size.width;
    }
  }
  return window_size;
}

static inline bool sc_screen_is_relative_mode(struct sc_screen *screen) {
  // screen->im.mp may be NULL if --no-control
  return screen->im.mp && screen->im.mp->relative_mode;
}

static void sc_screen_update_content_rect(struct sc_screen *screen) {
  assert(screen->video);
...
static void sc_screen_render(struct sc_screen *screen,
                             bool update_content_rect) {
  assert(screen->video);
  ...
  // Keymapper overlay: draw on top of the video
  km_render_overlay(screen->display.renderer, &screen->rect,
                    screen->frame_size);
  SDL_RenderPresent(screen->display.renderer);
}

static void sc_screen_render_novideo(struct sc_screen *screen) {
  enum sc_display_result res =
...
static bool sc_screen_frame_sink_open(struct sc_frame_sink *sink,
  ...
  if (ctx->width <= 0 || ctx->width > 0xFFFF || ctx->height <= 0 ||
      ctx->height > 0xFFFF) {
...
static void sc_screen_frame_sink_close(struct sc_frame_sink *sink) {
  struct sc_screen *screen = DOWNCAST(sink);
  (void)screen;
#ifndef NDEBUG
  screen->open = false;
...
static bool sc_screen_frame_sink_push(struct sc_frame_sink *sink,
                                      const AVFrame *frame) {
  struct sc_screen *screen = DOWNCAST(sink);
  assert(screen->video);
...
bool sc_screen_init(struct sc_screen *screen,
  ...
  ok = sc_display_init(&screen->display, screen->window, icon_novideo, mipmaps);
  if (icon) {
...
static void sc_screen_show_initial_window(struct sc_screen *screen) {
  ...
  struct sc_size window_size = get_initial_optimal_size(
      screen->content_size, screen->req.width, screen->req.height);
...
static void resize_for_content(struct sc_screen *screen,
                               struct sc_size old_content_size,
...
static void set_content_size(struct sc_screen *screen,
                             struct sc_size new_content_size) {
  assert(screen->video);
...
static void apply_pending_resize(struct sc_screen *screen) {
  assert(screen->video);
...
static bool sc_screen_init_size(struct sc_screen *screen) {
  // Before first frame
...
static enum sc_display_result prepare_for_frame(struct sc_screen *screen,
                                                struct sc_size new_frame_size) {
  assert(screen->video);
...
static bool sc_screen_apply_frame(struct sc_screen *screen) {
  assert(screen->video);
...
struct sc_point
sc_screen_convert_drawable_to_frame_coords(struct sc_screen *screen, int32_t x,
                                           int32_t y) {
  assert(screen->video);
...
struct sc_point
sc_screen_convert_window_to_frame_coords(struct sc_screen *screen, int32_t x,
                                         int32_t y) {
  sc_screen_hidpi_scale_coords(screen, &x, &y);
  return sc_screen_convert_drawable_to_frame_coords(screen, x, y);
}

void sc_screen_hidpi_scale_coords(struct sc_screen *screen, int32_t *x,
                                  int32_t *y) {
