...
static void
km_draw_char(SDL_Renderer *r, float x, float y, char c, float scale) {
    int idx = font_char_index(c);
    const uint8_t *glyph = FONT_5X7[idx];
    for (int row = 0; row < 7; row++) {
        for (int col = 0; col < 5; col++) {
            if (glyph[row] & (0x10 >> col)) {
                SDL_FRect px = {x + col * scale, y + row * scale, scale, scale};
...
static void
km_draw_text(SDL_Renderer *r, float x, float y, const char *text, float scale) {
    for (int i = 0; text[i]; i++) {
        km_draw_char(r, x + i * (6.0f * scale), y, text[i], scale);
    }
}

static float
km_text_width(const char *text, float scale) {
    int len = (int) strlen(text);
    return len > 0 ? (float) len * 6.0f * scale - scale : 0.0f;
}

// =====================================================
// Keycode/Mouse name converters (SDL3)
// =====================================================
static SDL_Keycode
km_name_to_keycode(const char *name) {
...
static uint8_t
km_name_to_mouse(const char *name) {
    if (!strcmp(name, "left"))
...
// =====================================================
static void
km_run_reload(void *userdata) {
    (void) userdata;
    km_reload_config();
}

static void
km_run_toggle_edit(void *userdata) {
    (void) userdata;
    km_toggle_edit();
}

static void
km_run_toggle_overlay(void *userdata) {
    (void) userdata;
    km_toggle_overlay();
}

static void
km_run_toggle_fps(void *userdata) {
    (void) userdata;
    km_toggle_fps(state.window);
}

static void
km_run_opac_up(void *userdata) {
    (void) userdata;
    km_adjust_opacity(0.1f);
}

static void
km_run_opac_down(void *userdata) {
    (void) userdata;
    km_adjust_opacity(-0.1f);
}

static Uint32 SDLCALL
km_ipc_timer_cb(void *userdata, SDL_TimerID timerID, Uint32 interval) {
    (void) userdata;
    (void) timerID;
    struct stat st;

    // Monitor keymap.cfg
    if (stat("keymap.cfg", &st) == 0) {
        static time_t last_cfg_time = 0;
        if (last_cfg_time == 0) {
            last_cfg_time = st.st_mtime;
        } else if (st.st_mtime > last_cfg_time) {
            last_cfg_time = st.st_mtime;
            sc_run_on_main_thread(km_run_reload, NULL, false);
        }
    }

    // Monitor keymap.cmd
    if (access("keymap.cmd", F_OK) == 0) {
        FILE *cmd_f = fopen("keymap.cmd", "r");
        if (cmd_f) {
            char cmd_str[64];
            if (fgets(cmd_str, sizeof(cmd_str), cmd_f)) {
                if (strncmp(cmd_str, "TOGGLE_EDIT", 11) == 0)
                    sc_run_on_main_thread(km_run_toggle_edit, NULL, false);
                else if (strncmp(cmd_str, "TOGGLE_OVERLAY", 14) == 0)
                    sc_run_on_main_thread(km_run_toggle_overlay, NULL, false);
                else if (strncmp(cmd_str, "TOGGLE_FPS", 10) == 0)
                    sc_run_on_main_thread(km_run_toggle_fps, NULL, false);
                else if (strncmp(cmd_str, "OPAC_UP", 7) == 0)
                    sc_run_on_main_thread(km_run_opac_up, NULL, false);
                else if (strncmp(cmd_str, "OPAC_DOWN", 9) == 0)
                    sc_run_on_main_thread(km_run_opac_down, NULL, false);
            }
            fclose(cmd_f);
        }
        remove("keymap.cmd");
...
void
km_set_window(SDL_Window *window) {
    state.window = window;
}
...
km_load_config(void) {
    ...
    while (fgets(line, sizeof(line), f) && state.count < KM_MAX_BINDINGS) {
        ...
        if (!strcmp(type_str, "KEY")) {
            ...
            b->type = KM_TYPE_DPAD;
            b->keycode = km_name_to_keycode(name_str); // placeholder, WASD
            float radius = 0.08f;
...
km_save_config(void) {
    ...
    for (int i = 0; i < state.count; i++) {
        ...
        const char *raw_name = (b->type != KM_TYPE_MOUSE) ? SDL_GetKeyName(b->keycode) : "";
        char lname[32];
        int j;
        for (j = 0; raw_name[j] && j < 30; j++)
            lname[j] = (raw_name[j] >= 'A' && raw_name[j] <= 'Z') ? raw_name[j] + 32 : raw_name[j];
        lname[j] = '\0';
...
// =====================================================
struct km_binding *
km_find_key(SDL_Keycode kc) {
    for (int i = 0; i < state.count; i++) {
        if (state.bindings[i].type == KM_TYPE_KEY && state.bindings[i].keycode == kc)
...
struct km_binding *
km_find_mouse_binding(uint8_t button) {
    for (int i = 0; i < state.count; i++) {
        if (state.bindings[i].type == KM_TYPE_MOUSE && state.bindings[i].mouse_button == button)
            return &state.bindings[i];
    }
    return NULL;
}
...
km_toggle_edit(void) {
    ...
    if (!state.edit_mode) {
        ...
        state.dragging = -1;
        state.selected = -1;
        LOGI("Keymapper: edit OFF, config saved.");
    } else {
        state.show_overlay = true;
...
void
km_toggle_overlay(void) {
    state.show_overlay = !state.show_overlay;
}

void
km_toggle_fps(SDL_Window *window) {
    if (window)
        state.window = window;
...
// =====================================================
// Overlay Rendering (SDL3 with FRect)
// =====================================================
void
km_render_overlay(SDL_Renderer *renderer, const struct SDL_FRect *cr,
    ...
    uint8_t alpha = (uint8_t) (state.opacity * 255.0f);

    for (int i = 0; i < state.count; i++) {
        struct km_binding *b = &state.bindings[i];

        float cx = cr->x + (cr->w * b->x_pct);
        float cy = cr->y + (cr->h * b->y_pct);
        ...
        strncpy(shortlabel, label ? label : "?", 6);
        shortlabel[6] = '\0';
        ...
        float ty = cy - 6.0f;
        SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
        km_draw_text(renderer, tx, ty, shortlabel, 2.0f);
    }
}

// =====================================================
// Edit mode mouse handling (SDL3)
// =====================================================
bool
km_handle_edit_mouse(const SDL_Event *event, const struct SDL_FRect *cr,
                     struct sc_size frame_size) {
    (void) frame_size;
    if (!state.edit_mode)
        return false;

    const float HITBOX_SQ = 45.0f * 45.0f;
    ...
    if (event->type == SDL_EVENT_MOUSE_MOTION && state.dragging >= 0) {
        float mx = event->motion.x;
        float my = event->motion.y;
        struct km_binding *b = &state.bindings[state.dragging];
        b->x_pct = (mx - cr->x) / cr->w;
        b->y_pct = (my - cr->y) / cr->h;
        if (b->x_pct < 0.0f)
            b->x_pct = 0.0f;
        if (b->x_pct > 1.0f)
            b->x_pct = 1.0f;
    ...
    if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN &&
        ...
        if (state.count < KM_MAX_BINDINGS) {
            float mx = event->button.x;
            float my = event->button.y;
            float nx = (mx - cr->x) / cr->w;
            float ny = (my - cr->y) / cr->h;
